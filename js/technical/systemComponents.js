var systemComponents = {
            ...
            <div class="upgRow">
                <div v-for="tab in Object.keys(data)">
                    <button v-if="data[tab].unlocked == undefined || data[tab].unlocked" 
                        ...
                        v-bind:style="[{'border-color': tmp[layer].color}, 
                            (data[tab].glowColor && subtabShouldNotify(layer, name, tab) ? 
                                {'box-shadow': 'var(--hqProperty2a), 0 0 20px ' + data[tab].glowColor} : {}), 
                            tmp[layer].componentStyles['tab-button'], data[tab].buttonStyle]"
    ...
    'overlay-head': {
        ...
        <div class="overlayThing" style="padding-bottom:7px; width: 90%; z-index: 1000; position: relative">
            <span v-if="player.devSpeed && player.devSpeed != 1" class="overlayThing">
                <br>Time speed: {{format(player.devSpeed)}}x<br>
            </span>
            <span v-if="player.offTime !== undefined" class="overlayThing">
                <br>Offline time remaining: {{formatTime(player.offTime.remain)}}<br>
            </span>
            <br>
    ...
    'info-tab': {
        ...
        <div>
            ...
            <br>
            Modding Tree <a v-bind:href="'https://github.com/Acamaeda/The-Modding-Tree/blob/master/changelog.md'" target="_blank" class="link" v-bind:style = "{'font-size': '14px', 'display': 'inline'}" >{{TMT_VERSION.tmtNum}}</a> by Acamaeda
            <br>
            Prestige Tree by Jacorb and Aarex
            <br><br>
            <div class="link" onclick="showTab('changelog-tab')">Modding Tree Changelog (Click version number for this tree's changelog)</div><br>
            <span v-if="modInfo.discordLink"><a class="link" v-bind:href="modInfo.discordLink" target="_blank">{{modInfo.discordName}}</a><br></span>
            <a class="link" href="https://discord.gg/F3xveHV" target="_blank" v-bind:style="modInfo.discordLink ? {'font-size': '16px'} : {}">Modding Tree Discord</a><br>
            <a class="link" href="http://discord.gg/wwQfgPa" target="_blank" v-bind:style="{'font-size': '16px'}">Prestige Tree Discord</a><br>
            <br><br>
    ...
    'options-tab': {
        ...
        <table>
            <tr>
                ...
                <td><button class="opt" onclick="hardReset()">Hard Reset (Delete Save)</button></td>
            </tr>
            <tr>
                <td><button class="opt" onclick="exportSave()">Export Save (Copy to Clipboard)</button></td>
                <td><button class="opt" onclick="importSave()">Import Save</button></td>
                <td><button class="opt" onclick="toggleOpt('offlineProd')">Offline Progress: {{ options.offlineProd?"ON":"OFF" }}</button></td>
            </tr>
            <tr>
                <td><button class="opt" onclick="switchTheme()">Theme: {{ getThemeName() }}</button></td>
                <td><button class="opt" onclick="adjustMSDisp()">Milestone Display: {{ MS_DISPLAYS[MS_SETTINGS.indexOf(options.msDisplay)]}}</button></td>
                <td><button class="opt" onclick="toggleOpt('hqTree')">HQ Tree: {{ options.hqTree?"ON":"OFF" }}</button></td>
            </tr>
            <tr>
                <td><button class="opt" onclick="toggleOpt('hideChallenges')">Completed Challenges: {{ options.hideChallenges?"HIDDEN":"SHOWN" }}</button></td>
                <td><button class="opt" onclick="toggleOpt('forceOneTab'); needsCanvasUpdate = true">Full-width Content: {{ options.forceOneTab?"ALWAYS":"AUTO" }}</button></td>
            </tr> 
        </table>`
    },
